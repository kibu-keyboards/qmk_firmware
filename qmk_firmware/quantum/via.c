...
//    raw_hid_send(packet, 32);
//}
// void get_screenlrgb_protocol_version(void) //Grab what version of the SignalRGB protocol a keyboard is running
// {
//    packet[0] = id_screenrgb_protocol_version;
//    packet[1] = PROTOCOL_VERSION_BYTE_1;
//    packet[2] = PROTOCOL_VERSION_BYTE_2;
//    packet[3] = PROTOCOL_VERSION_BYTE_3;
...
//    raw_hid_send(packet, 32);
// }
// void screen_led_streaming(uint8_t *data) //Stream data from HID Packets to Keyboard.
// {
//    uint8_t index = data[1];
//    uint8_t numberofleds = data[2];
...
//    }
// }
// void get_total_Screen_leds(void)//Grab total number of leds that a board has.
// {
//    packet[0] = id_screenrgb_get_total_leds;
//    packet[1] = (SIDE_LED_GROUP * SIDE_LED_SIZE);

//    raw_hid_send(packet, 32);
// }
// void get_screen_firmware_type(void) //Grab which fork of qmk a board is running.
// {
//    packet[0] = id_screenrgb_get_firmware_type;
//    packet[1] = FIRMWARE_TYPE_BYTE;
...
void raw_hid_receive(uint8_t *data, uint8_t length) {
    ...
    switch (*command_id) {
        ...
        //    break;
        //}
        // case id_screenrgb_protocol_version: {
        //     get_screenlrgb_protocol_version();
        //     break;
        // }
        // case id_screenrgb_stream_leds: {
        //    screen_led_streaming(data);
        //    break;
        // }
        // case id_screenrgb_get_total_leds: {
        //     get_total_Screen_leds();
        //     break;
        // }
        // case id_screenrgb_get_firmware_type: {
        //    get_screen_firmware_type();
        //    break;
        // }
        case id_emi_test: {
...
static uint8_t eeconfig_update_logo_flg = 0;
static uint16_t eeconfig_update_logo_Time = 0;

uint8_t Get_Write_Logo_Flash(void) {
    return eeconfig_update_logo_flg;
}

uint16_t Get_Write_Logo_Time(void) {
    return eeconfig_update_logo_Time;
}

void Updata_Logo_Flash_Time(void) {
    eeconfig_update_logo_Time = timer_read();
}
...
void via_qmk_rgblight_command(uint8_t *data, uint8_t length) {
    ...
    switch (*command_id) {
        ...
        case id_custom_save: {
            if (eeconfig_update_logo_flg == 0) {
                eeconfig_update_logo_flg = 1;
                Updata_Logo_Flash_Time();
            } else {
                Updata_Logo_Flash_Time();
            }
...
void via_qmk_rgblight_set_value(uint8_t *data) {
    ...
    switch (*value_id) {
        case id_qmk_rgblight_brightness: {
            ...
            rgblight_sethsv(rgblight_get_hue(), rgblight_get_sat(), ((uint16_t)value_data[0] * RGBLIGHT_LIMIT_VAL) / UINT8_MAX);
            break;
        }
        case id_qmk_rgblight_effect: {
            // if (value_data[0] == 0) {
            ...
            if (value_data[0] == 0) {
                ...
                rgblight_enable();
                rgblight_mode(value_data[0]);
            }
            break;
        }
        case id_qmk_rgblight_effect_speed: {
            // rgblight_set_speed_noeeprom(value_data[0]);
            rgblight_set_speed(value_data[0]);
            break;
        }
        case id_qmk_rgblight_color: {
            // rgblight_sethsv_noeeprom(value_data[0], value_data[1], rgblight_get_val());
            rgblight_sethsv(value_data[0], value_data[1], rgblight_get_val());
            break;
        }
    }
}
...
#if defined(RGB_MATRIX_ENABLE)
static uint8_t eeconfig_update_rgb_flg = 0;
static uint16_t eeconfig_update_rgb_Time = 0;

uint8_t Get_Write_Rgb_Flash(void) {
    return eeconfig_update_rgb_flg;
}

uint16_t Get_Write_Rgb_Time(void) {
    return eeconfig_update_rgb_Time;
}

void Updata_Rgb_Flash_Time(void) {
    eeconfig_update_rgb_Time = timer_read();
}
...
void via_qmk_rgb_matrix_command(uint8_t *data, uint8_t length) {
    ...
    switch (*command_id) {
        ...
        case id_custom_save: {
            if (eeconfig_update_rgb_flg == 0) {
                eeconfig_update_rgb_flg = 1;
                Updata_Rgb_Flash_Time();
            } else {
                Updata_Rgb_Flash_Time();
            }
...
void via_qmk_rgb_matrix_set_value(uint8_t *data) {
    ...
    switch (*value_id) {
        case id_qmk_rgb_matrix_brightness: {
            ...
            rgb_matrix_sethsv(rgb_matrix_get_hue(), rgb_matrix_get_sat(), User_scale8(value_data[0], RGB_MATRIX_MAXIMUM_BRIGHTNESS));
            break;
        }
        case id_qmk_rgb_matrix_effect: {
            // if (value_data[0] == 0) {
            ...
            if (value_data[0] == 0) {
                ...
                rgb_matrix_enable();
                rgb_matrix_mode(value_data[0]);
            }
            break;
        }
        case id_qmk_rgb_matrix_effect_speed: {
            // rgb_matrix_set_speed_noeeprom(value_data[0]);
            rgb_matrix_set_speed(value_data[0]);
            break;
        }
        case id_qmk_rgb_matrix_color: {
            // rgb_matrix_sethsv_noeeprom(value_data[0], value_data[1], rgb_matrix_get_val());
            rgb_matrix_sethsv(value_data[0], value_data[1], rgb_matrix_get_val());
            break;
        }
    }
}
...
static uint8_t eeconfig_update_side_flg = 0;
static uint16_t eeconfig_update_side_Time = 0;

uint8_t Get_Write_Side_Flash(void) {
    return eeconfig_update_side_flg;
}

uint16_t Get_Write_Side_Time(void) {
    return eeconfig_update_side_Time;
}

void Updata_Side_Flash_Time(void) {
    eeconfig_update_side_Time = timer_read();
}
...
void via_qmk_audio_command(uint8_t *data, uint8_t length) {
    ...
    switch (*command_id) {
        ...
        case id_custom_save: {
            if (eeconfig_update_side_flg == 0) {
                eeconfig_update_side_flg = 1;
                Updata_Side_Flash_Time();
            } else {
                Updata_Side_Flash_Time();
            }
            via_qmk_audio_save();
