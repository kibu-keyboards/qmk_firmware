 ...
 */
typedef struct {
  /** Initial value for ODT register.*/
  uint32_t      odt;
  /** Initial value for CFGLR register.*/
  uint32_t      cfglr;
...
/**
 * @brief   AT32 GPIO static initializer.
 ...
 * @notapi
 */
#define pal_lld_setport(port, bits) ((port)->SCR = (uint32_t)(bits))

/**
 * @brief   Clears a bits mask on a I/O port.
 * @details This function is implemented by writing the GPIO CLR register, the
 *          implementation has no side effects.
 * @note    Writing on pads programmed as pull-up or pull-down has the side
 *          effect to modify the resistor setting because the output latched
 *          data is used for the resistor selection.
 ...
 * @notapi
 */
#define pal_lld_clearport(port, bits) ((port)->CLR = (uint32_t)(bits))
